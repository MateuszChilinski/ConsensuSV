...
for sample in samples:
    ...
    process = Popen(cmd, shell=True, stdout=PIPE)
    all_charles = str(process.communicate()[0]).split("'")[1].split('\n')[0]
    print(all_charles)
    cmd = "grep -vc \"#\" output/fixed_HG00513.vcf"
    process = Popen(cmd, shell=True, stdout=PIPE)
    all_ours = str(process.communicate()[0]).split("'")[1].split('\n')[0]
    cmd = "grep -vc \"#\" uniq_comparison.vcf"
    process = Popen(cmd, shell=True, stdout=PIPE)
    all_intersect = str(process.communicate()[0]).split("'")[1].split('\n')[0]
